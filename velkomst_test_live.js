...
function nowOslo() {
  const d = new Date();
  return d.toLocaleTimeString("no-NO", { hour: "2-digit", minute: "2-digit", timeZone: "Europe/Oslo" }).replace(":", " ");
}
...
(async () => {
  try {
    if (!VOICE_ID_DEFAULT) throw new Error("Mangler ELEVENLABS_VOICE_ID i Secrets");
    const klokka = nowOslo();
    const vaer = await getWeather();
    const text = `${LANGUAGE_PRIMER} Velkommen hjem til Skilbrei. Lysa blir slått på for deg. Klokka er ${klokka}, og ute er det ${vaer}. I dag fortjener du å slappe av med en god kopp kaffe.`;
